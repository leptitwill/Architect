		<section class="content-admin">
			...
			<?php foreach ($etapes as $etape): ?>
				...
				<article class="faq">
					...
					<div class="modifier_faq">
						<a href="<?= base_url("admin/etape/modifier/$etape_id"); ?>" id="<?= $etape['idEtape'] ?>">
					...
					<div class="supprimer_faq">
						<span id="<?= $etape['idEtape'] ?>" onclick="supprimerQuestion(<?= $etape['idEtape'] ?>)">
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
					...
					<p class="faq_question"><?= $etape['titre'] ?></p>
					<p class="faq_separation">—</p>
					<p class="faq_reponse"><?= nl2br($etape['contenu']) ?></p>

				</article>

			<?php endforeach; ?>

			<a href="<?= base_url("admin/etape/creer"); ?>"><input type="button" class="button" value="Ajouter une étape"></a>
		...
		<script type="text/javascript">
			...
			function supprimerQuestion(id){
				swal({
					...
					text: "Une fois supprimé l\'étape ne sera plus disponible.",
					...
					if (isConfirm) {
				    	window.location.href = '<?= base_url() ?>admin/etape/supprimer/' + id;
					}
				})
			}
