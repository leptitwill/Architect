		<section class="content-admin">
			...
			<?php foreach ($solutions as $solution): ?>
				...
				<?php
					$solution_id = $solution['idSolution'];
				?>

				<article class="solution">

					<div class="modifier_solution">
						<a href="<?= base_url("admin/solution/modifier/$solution_id"); ?>" id="<?= $solution['idSolution'] ?>">
					...
					<div class="supprimer_solution">
						<span id="<?= $solution['idSolution'] ?>" onclick="supprimersolution(<?= $solution['idSolution'] ?>)">
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
					...
					<img class="solution_icone" src="<?=img_url()?>solution/<?= $solution['icone']; ?>">
					<div class="solution_info">
						<p class="solution_nom"><?= $solution['nom'] ?></p>
						<p class="solution_separation">—</p>
						<p class="solution_description"><?= $solution['description'] ?></p>
					</div>
			...
			<a href="<?= base_url("admin/solution/creer"); ?>"><input type="button" class="button" value="Ajouter une solution"></a>
		...
		<script type="text/javascript">
			...
			function supprimersolution(id){
				swal({
					...
					text: "Une fois supprimé la solution ne sera plus disponible.",
					...
					if (isConfirm) {
				    	window.location.href = '<?= base_url() ?>admin/solution/supprimer/' + id;
					}
				})
			}
