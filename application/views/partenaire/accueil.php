		<section class="content-admin">
			...
			<?php foreach ($partenaires as $partenaire): ?>
				...
				<article class="partenaire">
					...
					<div class="modifier_partenaire">
						<a href="<?= base_url("admin/partenaire/modifier/$partenaire_id"); ?>" id="<?= $partenaire_id ?>">
					...
					<div class="supprimer_partenaire">
						<span id="<?= $partenaire_id ?>" onclick="supprimerReseauSocial()">
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
					...
					<img class="partenaire_logo" src="<?=img_url()?>partenaire/<?= $partenaire['logo']; ?>">
					
					<div class="partenaire_info">
						<p class="partenaire_nom"><?= $partenaire['nom'] ?></p>
						<p class="partenaire_type"><?= $partenaire['type'] ?></p>
					</div>
			...
			<a style="display: block;" href="<?= base_url("admin/partenaire/creer"); ?>"><input type="button" class="button" value="Ajouter une partenaire"></a>
		...
		<script type="text/javascript">
			...
			function supprimerReseauSocial(){
				swal({
					...
					text: "Une fois supprimé le partenaire ne sera plus disponible.",
					...
					if (isConfirm) {
				    	window.location.href = '<?= base_url("admin/partenaire/supprimer/$partenaire_id") ?>';
					}
				})
			}
