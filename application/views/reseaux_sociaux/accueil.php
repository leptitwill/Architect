		<section class="content-admin">
			...
			<?php foreach ($reseaux_sociaux as $reseau_social): ?>
				...
				<article class="reseaux_sociaux">
					...
					<div class="modifier_reseaux_sociaux">
						<a href="<?= base_url("admin/reseaux_sociaux/modifier/$reseaux_sociaux_id"); ?>" id="<?= $reseaux_sociaux_id ?>">
					...
					<div class="supprimer_reseaux_sociaux">
						<span id="<?= $reseaux_sociaux_id ?>" onclick="supprimerReseauSocial(<?= $reseaux_sociaux_id ?>)">
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
					...
					<img class="reseaux_sociaux_logo" src="<?=img_url()?>reseaux_sociaux/<?= $reseau_social['logo']; ?>">
					
					<div class="reseaux_sociaux_info">
			...
			<a style="display: block;" href="<?= base_url("admin/reseaux_sociaux/creer"); ?>"><input type="button" class="button" value="Ajouter un réseau social"></a>

		</section>

		<script type="text/javascript">

			function supprimerReseauSocial(id){
				swal({
					title: 'Supprimer le réseau social ?',
					text: "Une fois supprimé le réseau social ne sera plus disponible.",
					...
					if (isConfirm) {
				    	window.location.href = '<?= base_url() ?>admin/reseaux_sociaux/supprimer/' + id;
					}
				})
			}
