		<section class="content-admin">
			...
			<?php foreach ($galeries as $galerie): ?>
				...
				<?php
					$galerie_id = $galerie['idGalerie'];
				?>

				<article class="galerie">

					<div class="modifier_galerie">
						<a href="<?= base_url("admin/galerie/modifier/$galerie_id"); ?>" id="<?= $galerie_id ?>">
					...
					<div class="supprimer_galerie">
						<span id="<?= $galerie_id ?>" onclick="supprimergalerie(<?= $galerie_id ?>)">
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
		...
		<script type="text/javascript">
			...
			function supprimergalerie(id){
				swal({
					title: 'Supprimer le galerie ?',
					text: "Une fois supprimé le galerie ne sera plus disponible.",
					...
					if (isConfirm) {
				    	window.location.href = '<?= base_url() ?>admin/galerie/supprimer/' + id;
					}
				})
			}
