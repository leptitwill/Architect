		<section class="content-admin">
			...
			<?php foreach ($avantages as $avantage): ?>
				...
				<?php
					$avantage_id = $avantage['idAvantage'];
				?>

				<article class="avantage">

					<div class="modifier_avantage">
						<a href="<?= base_url("admin/avantage/modifier/$avantage_id"); ?>" id="<?= $avantage['idAvantage'] ?>">
					...
					<div class="supprimer_avantage">
						<span id="<?= $avantage['idAvantage'] ?>" onclick="supprimerAvantage(<?= $avantage['idAvantage'] ?>)">
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
					...
					<img class="avantage_photo" src="<?=img_url()?>avantage/<?= $avantage['photo']; ?>">
					<div class="avantage_info">
						<p class="avantage_nom"><?= $avantage['prenom'] . ' ' . $avantage['nom']; ?></p>
						<p class="avantage_role"><?= $avantage['role'] ?></p>
						<p class="avantage_description"><?= $avantage['description'] ?></p>
					</div>
			...
			<a href="<?= base_url("admin/avantage/creer"); ?>"><input type="button" class="button" value="Ajouter un avantage"></a>

		</section>

		<script type="text/javascript">

			function supprimerAvantage(id){
				swal({
					title: 'Supprimer l\'avantage ?',
					text: "Une fois supprimé l\'avantage ne sera plus disponible.",
