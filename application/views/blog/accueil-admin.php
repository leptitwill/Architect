		<section class="content-admin">
			...
			<?php foreach ($articles as $article): ?>
				...
				<article class="produit">
					...
					<div class="modifier_produit">
						<a href="<?= base_url("admin/blog/modifier/$article_id"); ?>" id="<?= $article_id ?>">
					...
					<div class="supprimer_produit">
						<span id="<?= $article_id ?>" onclick="supprimerProduit(<?= $article_id ?>)">
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
					...
					<div class="produit_photo" style="background-image: url('<?=img_url()?>blog/<?= $article['couverture'] ?>')"></div>
					<div class="produit_info">
						<p class="produit_nom"><?= $article['nom'] ?></p>
						<a href="<?= base_url("produit/$article_url") ?>" class="produit_lien"><?= base_url("produit/$article_url") ?></a>
						<p class="produit_description"><?= nl2br($article['contenu']) ?></p>
					</div>
			...
			<a href="<?= base_url("admin/blog/creer"); ?>"><input type="button" class="button" value="Ajouter un article"></a>
		...
		<script type="text/javascript">
			...
			function supprimerProduit(id){
				swal({
					...
					text: "Une fois supprimé l\'article ne sera plus disponible.",
					...
					if (isConfirm) {
				    	window.location.href = '<?= base_url() ?>admin/blog/supprimer/' + id;
					}
				})
			}
