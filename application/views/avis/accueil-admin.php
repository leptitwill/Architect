		<section class="content-admin">
			...
			<?php foreach ($avis_clients as $avis_client): ?>
				...
				<?php
					$avis_id = $avis_client['idAvis'];
				?>

				<article class="avis">

					<div class="modifier_avis">
						<a href="<?= base_url("admin/avis/modifier/$avis_id"); ?>" id="<?= $avis_client['idAvis'] ?>">
					...
					<div class="supprimer_avis">
						<span id="<?= $avis_client['idAvis'] ?>" onclick="supprimerQuestion(<?= $avis_client['idAvis'] ?>)">
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
					...
					<p class="avis_question"><?= $avis_client['auteur'] ?></p>
					<p class="avis_separation">—</p>
					<p class="avis_reponse"><?= $avis_client['message'] ?></p>	

				</article>
			
			<?php endforeach; ?>

			<a href="<?= base_url("admin/avis/creer"); ?>"><input type="button" class="button" value="Ajouter un avis"></a>
		...
		<script type="text/javascript">
			...
			function supprimerQuestion(id){
				swal({
					...
					text: "Une fois supprimé l\'avis ne sera plus disponible.",
					...
					if (isConfirm) {
				    	window.location.href = '<?= base_url() ?>admin/avis/supprimer/' + id;
					}
				})
			}
